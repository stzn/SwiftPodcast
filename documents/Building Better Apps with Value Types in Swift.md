...
関数型プログラミングには不変性のある参照セマンティクスがある。これは、意図せぬ副作用など可変の参照セマンティクスが起こす多くの問題を解決する。

一方で不変な参照セマンティクスにもいくつかの問題がある。

- 可変のある世界が考えると、扱いづらいインターフェイスを定義することになるかもしれない
- マシンモデルの効率的にマップできない(最終的には ステートフルなCPUやキャッシュ、メモリなどにマップしなければならない)
...
- `NSDate`, `NSURL`, `UIImage`, `NSNumber`など(これらはコピーが不要)

**不変は良いこと**: 参照セマンティクスで何が起きているのかをわかりやすくするが、全体では完全に不変にすることはできない。
...
- 参照セマンティクスと予期せぬ変更はバグに繋がる
- 値セマンティクスは、可変性の表現性と不変性の安全性を提供することで、これらの問題を解決する　
